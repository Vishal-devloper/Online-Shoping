$(document).ready(function () {
        ...
        $.each(users, function (index, user) {
            let row = `
                <tr data-id="${user.id}">
                    ...
                    <td class="editable" data-field="brandName">${escapeHTML(user.brandName)}</td>
    ...
    // brand Toggle
    $(document).on("click",".addMoreBrand",function(){
        $(".brandInput").show();
        $("#cancelBrand").show();
        $(this).text("Save");
        $(this).addClass("saveBrand").removeClass("addMoreBrand");
        
    });
    $(document).on("click","#cancelBrand",function(){
        $(".brandInput").hide();
        $("#cancelBrand").hide();
        $(".saveBrand").addClass("addMoreBrand");
        $(".saveBrand").removeClass("saveBrand");
        $(".addMoreBrand").html(`<i class="fa-solid fa-plus"></i>`);
    });

    // Add brand
    $(document).on("click",".saveBrand",function(){
        let newBrand=$(".brandInput").val();
        if(newBrand!=""){
            $.ajax({
                url:_BASE_URL,
                data:{
                    function_name:"add_new_brand",
                    value:newBrand,
                    createdBy:adminId
                },
                method:"post",
